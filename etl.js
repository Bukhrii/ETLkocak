        ...
        await new Promise((resolve, reject) => {
            destDB.serialize(async () => {
                try {
                    ...
                    for (const table of tables) await runStatement(destDB, `DELETE FROM ${table}`);
                    ...
                    const allDates = [
                        ...
                        ...loans.map(l => l.origination_date),
                        ...digital_logs.map(d => d.date)
                    ].filter(Boolean);

                    for (const d_str of [...new Set(allDates)]) {
